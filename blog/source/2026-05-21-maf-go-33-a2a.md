...
- **A provider is just a way to produce responses** — here the "model" behind the agent is another agent on a remote server, reached over A2A. From the call site (`RunText`, middleware, sessions) it behaves like any `agent.Agent`; only what sits behind it changes.
- **Three moves:** resolve the remote agent's card from a URL (`agentcard.DefaultResolver.Resolve`), open an A2A client to a transport the card advertises (`a2aclient.NewFromCard`, forcing insecure gRPC here for the example), and wrap it with `a2aprovider.NewAgent`.
- **There is no LLM credential — the "credential" is an address.** `A2A_AGENT_HOST` locates the remote agent, which owns its own model and secrets; auth (if any) is negotiated per the agent card.
- The gRPC transport dials lazily, so the offline test builds a real client from an in-memory card without opening a socket; `a.ProviderName()` returns `"a2a"`.

## Further reading

- [a2a · Remote Skills as Function Tools](/blog/posts/maf-go-07-as-function-tools.html) — compose a remote agent's skills into a local host as tools.
- [A2A · Protocol Selection](/blog/posts/maf-go-09-protocol-selection.html) — pin which of the card's transports the client uses.
- [gRPC documentation](https://grpc.io/) — the transport this lesson forces (insecure, for the example only).
