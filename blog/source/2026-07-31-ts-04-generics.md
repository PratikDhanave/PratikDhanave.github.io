...
(One honest caveat: `arr[0]` on an *empty* array is `undefined` at runtime, so the truly safe signature is `first<T>(arr: T[]): T | undefined` — and with the `noUncheckedIndexedAccess` compiler option on, TypeScript infers exactly that and forces you to handle the empty case. We keep `: T` here to focus on the generic mechanism, but a production version should return `T | undefined`.)
