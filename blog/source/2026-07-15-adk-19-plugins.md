...
- A plugin is not a new kind of hook — it reuses the Module 09 callback set but registers *once on the Runner*, firing for every agent, model call, tool call, and event in the app.
- The same short-circuit contract applies globally: returning `None`/`nil` observes only; returning a value overrides (a `before_model` plugin returning a response skips the model app-wide; an `on_event` plugin returning an event replaces it).
- The Python/Go idiom split recurs: Python subclasses `BasePlugin` and overrides coroutines; Go composes a `plugin.Config` struct of callback fields, leaving unused ones `nil`. Both register on the Runner (`plugins=[...]` / `PluginConfig`), and behavior is identical.
- Decide by scope: one agent → callback (keep it local); every agent → plugin (logging, cost meters, rate limits, global instructions, caches), and it covers agents you add later.
- Both SDKs ship a `LoggingPlugin` and a reflect-and-retry plugin, but there's no prebuilt PII redactor or content filter — lift the Module 09 guardrail pattern onto the Runner for those.
