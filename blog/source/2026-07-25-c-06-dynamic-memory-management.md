...
int *tmp = realloc(arr, 2 * n * sizeof(int)); // resize; realloc into a TEMP
if (tmp == NULL) { /* arr is still valid — handle, don't lose it */ }
else { arr = tmp; }                           // only reassign on success (see leak trap below)
