...
- Your agent code doesn't change going to production — only packaging and the command do. ADK is deployment-agnostic across Cloud Run, GKE, and Vertex Agent Engine.
- What the deployable *is* differs by language: in Python a FastAPI app (`get_fast_api_app`) is what Cloud Run runs; in Go the compiled binary *is* the server, shipping in a tiny distroless image (tens of MB, no shell) versus Python's hundreds of MB — a real cold-start and image-scan edge.
- `adk deploy cloud_run` collapses build, push, and deploy into one command; `adk deploy agent_engine` goes further and provisions managed sessions and memory so you run no database or bucket yourself.
- The single production change is persistence: point session/memory/artifact services at real backends (managed for you on Agent Engine).
- For event-driven ("ambient") agents, two rules make at-least-once delivery safe: derive the session id from a stable event field so redelivery reuses the session, and **ack after the invocation succeeds**, not on receipt.

## Further reading

- [Observability in ADK: seeing inside a running agent](/blog/posts/adk-14-observability.html)
- [ADK to MAF: deployment](/blog/posts/adk-to-maf-deployment.html)
