...
- The four context types are capability tiers, not unrelated classes: `ReadonlyContext` ⊂ `CallbackContext` ⊂ `ToolContext`, with `InvocationContext` carrying the raw runtime services — in `google-adk` 2.x `ToolContext is CallbackContext` is literally `True`.
- The runtime hands you the narrowest type appropriate to the call site — read-only in instruction providers, mutable state and artifacts in callbacks, actions/memory/credentials in tools — and you don't get to choose your ring.
- Instruction providers get a `ReadonlyContext` by design: prompt building must be a pure function of current state, because a provider that mutated state would make runs non-reproducible.
- The practical rule: when you reach for a capability the context doesn't offer, that's a signal you're doing the work in the wrong place (write state in a callback/tool, escalate via `ToolContext.actions`).
- Go models the same hierarchy with embedded interfaces rather than subclassing, and either language lets you introspect the exact injected context offline through an in-memory runner with no model call.

## Further reading

- [Artifacts in ADK](/blog/posts/adk-07-artifacts.html)
- [Callbacks in ADK](/blog/posts/adk-09-callbacks.html) — the lifecycle hooks built on `CallbackContext`
- [Sessions & State in ADK](/blog/posts/adk-05-sessions-and-state.html)
