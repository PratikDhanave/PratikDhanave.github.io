...
- In the common case you don't build a tool object — Python passes the function directly (ADK reads name, docstring, and type hints), while Go declares the schema via a typed input struct plus a `functiontool.Config`.
- The docstring is the contract the model reads to decide *when* and *how* to call a tool; treat it like an API spec and return structured values (`{"status": "ok", "cart_size": 2}`), not a bare `True`.
- `ToolContext` (Python) / `ctx.State()` (Go) reaches session state — arguments are what the *model* supplies per call, state is what *your app* remembers between calls; Python strips the injected `tool_context` from the schema.
- A long-running tool returns *before* the work finishes, emitting a pending marker and pausing the run; human-in-the-loop and "wait for an async backend" ride the same machinery — only *who* completes the call differs.
- A Go gotcha: the Gemini API won't mix built-in `GoogleSearch` with custom function tools in one agent — isolate each kind in its own sub-agent.

## Further reading

- [Multi-Agent Systems in ADK](/blog/posts/adk-03-multi-agent-systems.html)
- [Sessions and State](/blog/posts/adk-05-sessions-and-state.html) — the store these tools read and write
