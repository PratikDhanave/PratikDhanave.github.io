...
- Every model route in ADK ends at one tiny interface — `BaseLlm` in Python, `model.LLM` in Go — and an `LlmAgent` never knows which provider actually answered.
- Python is flexible: `LlmAgent(model=...)` takes a string (the Gemini path via `LLMRegistry`), or a `BaseLlm` object like `LiteLlm(...)` reaching 100+ providers and sidestepping the registry — when in doubt, pass an object.
- Go is uniform: `llmagent.Config.Model` is typed `model.LLM`, so you always construct and pass a value; there's no bare-string shortcut and no `LLMRegistry` in the v2 module.
- Constructing a model (e.g. `LiteLlm(...)`) is offline — it wires config but reaches no network — so you can build and validate an agent's wiring without credentials.
- A fake model plugged into a *real* agent runs the whole pipeline offline; that's model-agnosticism made concrete, and it's exactly how ADK's own tests drive agents. Treat the model like a database driver: depend on the interface, keep the vendor a swappable config detail.

## Further reading

- [Plugins in Google ADK: One Hook, Every Agent](/blog/posts/adk-19-plugins.html)
- [Grounding & RAG in ADK](/blog/posts/adk-17-grounding-and-rag.html)
