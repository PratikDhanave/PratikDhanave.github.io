...
- Memory and state are different tools: state is a small, exact key lookup that lives inside one session; memory is a searchable archive that persists indefinitely across all sessions and is queried by *relevance*, not by key.
- A `MemoryService` has exactly two verbs — `add_session_to_memory(session)` ingests a *finished* conversation and indexes it, and `search_memory(query)` returns the most relevant snippets — and every backend implements the same two.
- In a live agent you rarely call `search_memory` by hand; you give the agent a `load_memory` tool (or `preload_memory` to inject automatically) so the model decides when to reach back, while a background job ingests each finished session.
- The in-memory backend does keyword matching (why "favorite color" hits and the unrelated query misses); Vertex AI Memory Bank / RAG backends do semantic embedding search — you swap only the constructor, the agent code is untouched.
- `MemoryService` is an interface (`BaseMemoryService` in Python, `memory.Service` in Go), so you can back it with any external vector DB by embedding on ingest and similarity-searching on recall; the two verbs stay constant.

## Further reading

- [Sessions & State in ADK](/blog/posts/adk-05-sessions-and-state.html) — the within-conversation counterpart
- [Artifacts in ADK](/blog/posts/adk-07-artifacts.html) — the next post in the series
- [Grounding and RAG in ADK](/blog/posts/adk-17-grounding-and-rag.html)
