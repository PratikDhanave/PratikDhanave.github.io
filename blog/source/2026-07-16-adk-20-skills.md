...
- A skill is a folder with a `SKILL.md` (YAML frontmatter — at minimum `name` and `description` — plus a Markdown body) and optional bundled resources; the capability is a file tree, not code, so it's inspectable, versionable, and portable.
- The design turns on cheap discovery, on-demand load: every skill's one-line `description` is read up front so the model can choose, but the full body and resources are read only for the one it picks — keeping the prompt small across dozens of skills.
- Python hands you one fully-populated `Skill` object (`list_skills_in_dir` / `load_skill_from_dir`); Go models a `Source` you read piece by piece (`LoadFrontmatter`, `LoadInstructions`, `ListResources`, `LoadResource`), with optional preload wrappers for eager loading.
- Two cross-language asymmetries: resource keys differ (Python `"cities.txt"` vs. Go `"references/cities.txt"`), and Go's YAML parser rejects unknown top-level keys — stick to documented fields for one `SKILL.md` to load in both.
- Prefer a plain function tool for one clean operation; reach for a skill to share a capability across agents, to keep large-but-sometimes-relevant instructions out of the base prompt, or for inspectable/versionable capabilities. Directory name must match frontmatter `name`, and a malformed `SKILL.md` is a load-time error.

## Further reading

- [Google ADK Code Execution](/blog/posts/adk-21-code-execution.html)
- [Plugins in Google ADK: One Hook, Every Agent](/blog/posts/adk-19-plugins.html)
